...
void region_save(region_t *region)
{
  ...
  for (int i = 0; i < REGION_CHUNKS_LENGTH; i++)
  {
    ...
    {
      ivec voxelIndex = index3d(i, VOXELS_CROOT);
      ...
      ivec nextVoxel = index3d(i + 1, VOXELS_CROOT);
      if (voxelValue !=
          region->chunks[chunkIndex.x][chunkIndex.y][chunkIndex.z].voxels[nextVoxel.x][nextVoxel.y][nextVoxel.z])
      {
        buffer.value = voxelValue;
        fwrite((const void *)&buffer, sizeof(buffer), 1, file);
        lastWriteIndex = i;
...
region_t region_load(ivec cord)
{
  ...
  for (int i = 0; i < REGION_CHUNKS_LENGTH; i++)
  {
    ...
    while (voxel < VOXELS_LENGTH)
    {
      ...
      {
        ivec index = index3d(voxel, VOXELS_CROOT);
...
void region_set_pos(region_t *region, ivec cord)
{
  ...
  for (int i = 0; i < REGION_CHUNKS_LENGTH; i++)
  {
    ...
    chunk->pos = vec{position.x * VOXELS_CROOT, position.y * VOXELS_CROOT, position.z * VOXELS_CROOT} + region->pos;
    chunk->update = true;
  }
}