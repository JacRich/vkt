...
mesh_t mesh_load_obj(char const* path, shader_t shader, texture_t* texture, int drawflags)
{
  ...
  while(!feof(file)) 
    ...
    int indexBuffer[9];
    if(sscanf(linebuf, "f %i/%i/%i %i/%i/%i %i/%i/%i", &indexBuffer[0], &indexBuffer[1], &indexBuffer[2], &indexBuffer[3], &indexBuffer[4], &indexBuffer[5], &indexBuffer[6], &indexBuffer[7], &indexBuffer[8])){
      for(int i = 0; i < 9; i += 3){
