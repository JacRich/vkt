...
mesh_t* region_bounds[REGION_COUNT];
transform_t* region_tran[REGION_COUNT];
...
void veng_init()
{
  ...
  for (int i = 0; i < REGION_COUNT; i++)
  {
    ...
    entity_t region_bound = entity_add(C_MESH | C_TRANSFORM);
    region_bounds[i] = &components.meshes[region_bound];
    *region_bounds[i] = mesh_load_obj("gamedata/editor_region_bounds.obj", sh_cursor, NULL, 0);
    region_tran[i] = &components.transforms[region_bound];
    region_tran[i]->scale = {128.0f, 128.0f, 128.0f};
    region_tran[i]->pos = regions[i].pos;
  }
