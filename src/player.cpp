...
static void key_callback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
  ...
  {
    int newMode = glfwGetInputMode(window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED ? GLFW_CURSOR_NORMAL
...
void player_tick()
{
  ...
  player.pitch += yDelta;
  player.pitch = player.pitch > 89.0f ? 89.0f : player.pitch < -89.0f ? -89.0f
                                                                      : player.pitch;
